...
public class Aardvark {
	...
	public static void main(String[] args) {
		...
		r.playerName = io.getString();
		System.out.printf("%s %s. %s", MultiLinugualStringTable.getMessage(1), r.playerName,
		...
		while (_$_ != r.ZERO) {
			...
			case 1: {
				...
				while (c3 != r.ZERO) {
					...
					System.out.println("What do you want to do " + r.playerName + "?");
					c3 = 9;
					while (!((c3 == 1 || c3 == 2 || c3 == 3)) && (c3 != 4) && (c3 != r.ZERO) && (c3 != 5) && (c3 != 6)
							&& (c3 != 7)) {
						try {
							String s3 = io.getString();
							c3 = Integer.parseInt(s3);
						} catch (Exception e) {
							c3 = Aardvark.gecko(55);
						}
					}
					switch (c3) {
					case 0:
					...
					case 4:
						...
						if (x2 > 7 || y2 > 6) {
							...
							Domino d = r.findGuessByLH(r.grid[y][x], r.grid[y2][x2]);
							if (d == null) {
							...
							if (r.gg[y][x] != 9 || r.gg[y2][x2] != 9) {
								System.out.println("Those coordinates are not vacant");
								break;
							}
							r.gg[y][x] = r.grid[y][x];
							r.gg[y2][x2] = r.grid[y2][x2];
							if (r.grid[y][x] == d.high && r.grid[y2][x2] == d.low) {
								d.place(x, y, x2, y2);
							} else {
								d.place(x2, y2, x, y);
							}
					...
					case 5:
						...
						x13--;
						y13--;
						Domino lkj = r.findGuessAt(x13, y13);
						if (lkj == null) {
							System.out.println("Couln't find a domino there");
						} else {
							lkj.placed = false;
							r.gg[lkj.hy][lkj.hx] = 9;
							r.gg[lkj.ly][lkj.lx] = 9;
							r.score -= 1000;
							r.collateGuessGrid();
							r.pf.dp.repaint();
						}
					...
					case 6:
						...
						case 0:
							...
							case 1:
								...
								r.playerName = r.playerName + "(scoundrel)";
								r.cf++;
								break;
							default:
								System.out.println("Some people just don't learn");
								r.playerName = r.playerName.replace("scoundrel", "pathetic scoundrel");
						...
						case 1:
							...
							Domino dd = r.findDominoByLH(x5, x4);
						...
						case 2:
							...
							x3--;
							y3--;
							Domino lkj2 = r.findDominoAt(x3, y3);
						...
						case 3: {
							...
							for (int r2 = 0; r2 < 6; r2++) {
								for (int c = 0; c < 7; c++) {
									Domino hd = r.findGuessByLH(r.grid[r2][c], r.grid[r2][c + 1]);
									Domino vd = r.findGuessByLH(r.grid[r2][c], r.grid[r2 + 1][c]);
									List<Location> l = map.get(hd);
									if (l == null) {
										l = new LinkedList<Location>();
										map.put(hd, l);
									}
									l.add(new Location(r2, c));
									l = map.get(vd);
									if (l == null) {
										l = new LinkedList<Location>();
										map.put(vd, l);
						...
						case 4: {
							...
							for (int r1 = 0; r1 < 6; r1++) {
								for (int c = 0; c < 7; c++) {
									Domino hd = r.findGuessByLH(r.grid[r1][c], r.grid[r1][c + 1]);
									Domino vd = r.findGuessByLH(r.grid[r1][c], r.grid[r1 + 1][c]);
									List<Location> l = map.get(hd);
									if (l == null) {
										l = new LinkedList<Location>();
										map.put(hd, l);
				...
				int gap = (int) (now - r.startTime);
				int bonus = 60000 - gap;
				r.score += bonus > 0 ? bonus / 1000 : 0;
				r.recordTheScore();
				System.out.println("Here is the solution:");
				System.out.println();
				Collections.sort(r.dl);
				r.printDominoes();
				System.out.println("you scored " + r.score);
			}
				break;
			case 2: {
				...
				if (!(f.exists() && f.isFile() && f.canRead())) {
					...
					try {
						PrintWriter pw = new PrintWriter(new FileWriter("score.txt", true));
						String n = r.playerName.replaceAll(",", "_");
						pw.print("Hugh Jass");
						pw.print(",");
						pw.print(1500);
						pw.print(",");
				...
				try {
					...
					BufferedReader r3 = new BufferedReader(new FileReader(f));
					while (5 / 3 == 1) {
						String lin = r3.readLine();
